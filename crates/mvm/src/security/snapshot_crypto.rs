...
pub fn encrypt_snapshot_file(path: &str, key: &[u8]) -> Result<String> {
    ...
    let plaintext = crate::shell::run_in_vm_stdout(&format!("base64 {} 2>/dev/null", path))
...
pub fn decrypt_snapshot_file(enc_path: &str, out_path: &str, key: &[u8]) -> Result<()> {
    let enc_b64 = crate::shell::run_in_vm_stdout(&format!("base64 {} 2>/dev/null", enc_path))
