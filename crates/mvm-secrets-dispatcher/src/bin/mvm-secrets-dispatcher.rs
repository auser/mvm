//! `mvm-secrets-dispatcher` binary — the secrets dispatcher subprocess
//! entry point (Plan 104 §H-L1, ADR-061 §"Decision"; ADR-049 §"Substitution
//! flow" for the host.secrets.v1 design).
//!
//! Spawn contract (W1a):
//!
...
use mvm_secrets_dispatcher::config::{SubprocessConfig, parse as parse_config};
use mvm_secrets_dispatcher::registry::Registry;
use mvm_secrets_dispatcher::server::serve_on_listener;
...
fn read_stdin_blocking() -> Result<Vec<u8>> {
    ...
    std::io::stdin()
        .lock()
        .read_to_end(&mut buf)
        .context("mvm-secrets-dispatcher stdin read failed")?;
    Ok(buf)
}
...
fn main() -> Result<()> {
    ...
    let cfg: SubprocessConfig =
        parse_config(&raw).context("mvm-secrets-dispatcher config parse failed")?;
    info!(
        workload_id = %cfg.workload_id,
        tenant_id = %cfg.tenant_id,
        uds_path = %cfg.uds_path.display(),
        max_frame_bytes = cfg.max_frame_bytes,
        "mvm-secrets-dispatcher config loaded"
    );

    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .worker_threads(2)
        .thread_name("mvm-secrets-dispatcher")
        .build()
        .context("mvm-secrets-dispatcher tokio runtime build failed")?;

    runtime.block_on(async move {
        let listener = UnixListener::bind(&cfg.uds_path)
            .with_context(|| format!("mvm-secrets-dispatcher UDS bind failed on {}", cfg.uds_path.display()))?;
        let registry = Arc::new(Registry::new());
        info!(
            uds_path = %cfg.uds_path.display(),
            "mvm-secrets-dispatcher listening; HostSecretsV1Handler not yet registered (W1b.1 scaffold — every call returns NotBound until W5 wires ADR-049)"
        );
        ...
        .await
        {
            error!(error = %e, "mvm-secrets-dispatcher serve loop exited with error");
            return Err::<(), _>(e);
        }
        Ok(())
    })?;
