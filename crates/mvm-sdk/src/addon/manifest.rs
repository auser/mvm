...
// allow(secret-debug): variants are *kind* descriptors (which
// credential strategy to use), not credential values. `Generated`
// carries no bytes; future variants like `Static` carry the
// already-published-and-public reference, not the credential itself.
// Debug-printing is safe and useful for diagnostics.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "snake_case")]
#[non_exhaustive]
