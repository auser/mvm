...
- **Host Nix is never used by `mvmctl` at runtime**, even when present: `mvmctl` does not shell out to a host `nix` binary, does not consult `nix-darwin`'s `linux-builder`, and does not honor `nix-daemon` URLs in any code path. Every Nix evaluation goes through a VM we launched; builds run inside that builder VM via libkrun (macOS) or Firecracker (Linux). The reason is determinism and consistency: the same `mvmctl` produces the same artifacts on every host regardless of what the host happens to have installed. A contributor with host Nix installed must not see different runtime behavior from a contributor without it. This invariant supersedes ADR-013's "host Nix remains an opt-in power-user path" clause for everything inside `mvmctl` *at runtime*.
   - **The one-time contributor bootstrap step is the exception, and it lives outside `mvmctl`**. `cargo xtask build-dev-image --arch <arch>` (the maintainer-side seeder, see `xtask/src/build_dev_image.rs`) uses host Nix to produce the bytes that land in the gitignored `nix/images/dev-prebuilt/<arch>/` vendored slot. Plan 77 W7 wires Stage 0 to read that slot, so once a contributor has run xtask once, every subsequent `mvmctl dev up` rebuilds via libkrun and never touches host Nix again. xtask is a contributor setup tool — not part of `mvmctl`'s runtime — so the runtime invariant above stays true.
- **Two-track build vs. release**: the dev track (contributor source checkout) builds the builder VM and dev image from in-tree flakes via libkrun, after the one-time xtask bootstrap above. The release track (installed-binary users) downloads + cosign-verifies pre-built `dev-rootfs-*.ext4` + builder-VM artifacts the release pipeline cuts on a Linux Nix runner. The two tracks share *no* artifact path: contributor `mvmctl dev up` never downloads a release prebuilt, and installed-binary `mvmctl dev up` never invokes host Nix. See Plan 77 W7 (`specs/plans/77-stage0-bootstrap-via-dev-image.md`) for the resolution of the chicken-and-egg this design closes.
