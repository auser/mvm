...
- **No prebuilt builder VM artifact is downloaded when developing locally — ever — and none is published until we cut a release.** The builder VM image is local-build only on contributor hosts: source checkout → local Stage 0 (libkrun + the cached dev image as the seed Linux env) → local builder VM image at `~/.cache/mvm/builder-vm/<arch>/`. `bootstrap_builder_vm_image` must not reach a `download_builder_vm_image` path on source-checkout hosts under any circumstances. The release artifact for the builder VM does not exist; do not add code that assumes it does. This is a stricter invariant than "source-checkout builds never depend on mvm-published artifacts" — it also bans the *fallback* download path, and bans publishing the artifact in CI until a release is explicitly cut. Until that point, `bootstrap_builder_vm_image` must build locally or fail with an actionable error; it must never attempt a network fetch for the builder VM image.
