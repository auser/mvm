...
**Goal:** Implement ADR-067 — a raw secret value never enters the guest. The workload routes a secret-bearing request through a host substitution endpoint with an opaque placeholder; the host injects (bearer) or signs (SigV4/HMAC) and makes the real TLS; values come from a pluggable resolver (local OS keyring or `mvmd`), with a software-first keyholder (hardware-sealed when present). Restores claims 12 + 13.
