...
- [x] **W3 — Doctor probe.** Shipped via Plan 105 W3-doctor in the same PR as W1 prep (PR #479). `mvmctl doctor` now emits a `nested-kvm` line driven by `Platform::has_nested_kvm()` (`crates/mvm-core/src/platform/platform.rs`) which probes both `/sys/module/kvm_intel/parameters/nested` and `/sys/module/kvm_amd/parameters/nested`. The check reports one of four states — env-set+ready, env-set+missing (hard fail with kernel-module fix command), env-unset+ready, env-unset+missing — and the `builder backend` line gains a `$MVM_LINUX_BUILDER_VM=1 (Plan 100 W6 opt-in)` annotation when the env is set.
