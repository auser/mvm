...
- `crates/mvm/src/vm/{firecracker,apple_container,docker,microvm_nix}.rs` — implement `security_profile()` per backend
- `crates/mvm/src/vm/backend.rs` — `AnyBackend::security_profile()` dispatch
...
**Files**:
- `crates/mvm/src/vm/cow.rs` (new) — `reflink_or_err`, `reflink_or_copy`
- `crates/mvm/src/vm/template/lifecycle.rs` — add `template_clone_for_instance(template_id, instance_dir) -> Result<CloneStrategy>`
- `crates/mvm/src/vm/apple_container.rs` — call clone in `start()` before pointing VZ at the disk
- `crates/mvm/src/vm/firecracker.rs` — opportunistic clone on Linux
- `crates/mvm/src/vm/mod.rs` — export `cow` module
- `crates/mvm/Cargo.toml` — `libc.workspace = true` already present; no new deps
...
- `crates/mvm/src/vm/libkrun.rs` (new) — `LibkrunBackend` impl
- `crates/mvm/src/vm/backend.rs` — add `Libkrun(LibkrunBackend)` variant + `auto_select` rules
...
| `crates/mvm/src/vm/backend.rs` | E (new variant), B (profile dispatch) |
| `crates/mvm/src/vm/{firecracker,apple_container,docker,microvm_nix}.rs` | B (security profiles), D (CoW for FC + AC) |
| `crates/mvm/src/vm/cow.rs` (new) | D |
| `crates/mvm/src/vm/template/lifecycle.rs` | D |
| `crates/mvm/src/vm/libkrun.rs` (new) | E |
