...
Owner requirement: the SDKs are **generated from one schema**, not hand-maintained, so Python / TS / Rust can't drift from each other or from the core. The foundation exists — the IR types derive `JsonSchema` and `mvm-ir/src/bin/emit_schema.rs` emits the schema; the host↔guest protocol gets the same. Generate, in every language: the **IR / data types** *and* the **RPC client method surface** (one host↔guest RPC → one generated client method). The idiomatic veneer (the `Sandbox` mode-switching, the decorator AST hooks, the typed helpers — plan 125) stays hand-written *over* the generated core; it's small and carries no wire contract, so it can't drift the protocol.
