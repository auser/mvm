...
- **A.2 — whiteouts and opaque directories.** OCI v1.1 `.wh.<name>` file removes `<name>` from the assembled tree at this layer. `.wh..wh..opq` clears the parent directory's prior layer content. Whiteout-path traversal refused with the same strictness as A.1.
