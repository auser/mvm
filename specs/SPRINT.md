...
- **W6.1.2** — cross-compile the real `mvm-guest-agent` Rust binary (~2400 LOC) for target Linux and bake it into the rootfs. The supervision plumbing is ready; only the binary inclusion is missing. Needs a Linux builder (nix-darwin's linux-builder or remote nix-daemon); deferred because the cross-compile dep is non-trivial and orthogonal to the supervision shape this wave shipped.
