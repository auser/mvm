...
- [ ] Follow-up PR after the local-volume encryption gate: make encryption an mvm-owned per-volume property instead of only a host-backing precondition. `mvmctl volume create` should provision an encrypted volume container or equivalent encrypted backing, persist per-volume encrypted metadata, require explicit unlock/key release before mount, keep plaintext exposure scoped to the microVM mount lifecycle, and add security tests for wrong key, missing key, tampered metadata, locked volume mount refusal, and clean unmount/lock behavior.
