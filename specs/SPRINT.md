...
- [x] **W7 — backend extraction (alt backends + handle registry)** ✅ — landed on `feat/w7-backend-extraction` (4 commits, 2026-05-10). New `mvm-runtime-base` crate carries the leaf substrate (`ui`, `runtime_meta`, `cow`); the 5 alt backends (`apple_container`, `cloud_hypervisor`, `docker`, `libkrun`, `microsandbox`) moved out of `mvm-runtime/src/vm/` into `mvm-backend/src/`; the dependency direction flipped from `mvm-backend → mvm-runtime` (re-export façade) to `mvm-runtime → mvm-backend` (consumer). Handle registry in `mvm-backend::handle_registry` closes the W6.2-era gap where `StartMode::Attached` was intent-only metadata: `mvmctl up --attached` now teardowns the sandbox on Ctrl-C via the CLI's existing top-level signal handler. `cargo test --workspace --no-fail-fast` 1895/0; clippy clean. mvmd contract gate not run (pre-existing `microsandbox 0.4.5` ⊥ `iroh-base 0.96.1` over `sha2`).
- [ ] **W6.1.2** — cross-compile the real `mvm-guest-agent` Rust binary (~2400 LOC) for target Linux and bake it into the rootfs. The W6.1.1 supervision pattern is in; only the binary swap is missing. Needs a Linux builder. **Unlocked once W7.x.2 lands** (in-VM builder removes the host-Nix dep).
- [ ] **W8 — Firecracker direct-launch (drop Lima from `vm::microvm`)**. The W7 split deliberately left `firecracker.rs`, `microvm.rs`, `microvm_nix.rs`, `vm::image`, and the `AnyBackend`/`FirecrackerBackend` impl in `mvm-runtime` because they all path through `run_in_vm` → `linux_env::default_env()` → Lima on macOS / no-KVM Linux. The host-side Lima shim is gone (slice 2 of W7 deleted `Platform::needs_lima`, `bootstrap::is_lima_required`, `inside_lima`, and `vm/lima.rs`), so `mvmctl dev` already bails with a clear "see W8" message on non-Apple-Container hosts and `commands/env/setup.rs` skips the Lima setup step. What W8 does:
    1. Replace `firecracker::install`/`download_assets`/`prepare_rootfs`/`write_state` with a host-side direct invocation (downloads land in `~/.cache/mvm/firecracker/`, kernel + rootfs in `~/.local/share/mvm/microvm/`).
    2. Replace `microvm::run_from_build`/`stop_vm`/`list_vms` with direct host-side spawn of the `firecracker` binary against per-VM dirs under `~/.local/share/mvm/vms/<name>/`.
    3. Drop `MICROVM_DIR` / `VMS_DIR` / `run_in_vm` / `linux_env::LimaEnv` and the rest of the `shell::run_on_vm*` family. `shell::run_host*` stays (real host operations) and probably moves to `mvm-runtime-base` as part of the move.
    4. Move `firecracker.rs` + `microvm.rs` + `microvm_nix.rs` + `vm/image.rs` + `vm/backend.rs` + `vm/network.rs` (and any of `bridge`, `disk_manager`, `egress_proxy`, `instance_snapshot`, `name_registry`, `volume_registry`, `vminitd_client` that turn out to need it) from `mvm-runtime` into `mvm-backend`. After that, `AnyBackend` and `FirecrackerBackend` live in `mvm-backend`; `mvm-runtime` becomes the thin orchestration layer it was always meant to be.
    5. Wire up the non-Apple-Container `mvmctl dev` path against the same direct Firecracker launcher (replaces the deleted Lima `dev_up`/`dev_down`/`dev_status`).
    6. Update `mvm-runtime/src/vm/mod.rs` to drop the alt-backend re-exports added in W7 slice 3 once consumers in `mvm-cli` migrate to `mvm_backend::*` directly.
- [ ] **Phase 1 close-out** — demo run + checkpoint review against `mvm-runtime` Phase 1 exit tests in plan 60.

#### Architectural completion of W6.2.x (smaller follow-ups)
- [ ] **W6.2.3 follow-up** — add `BuildModeFlags` to `mvmctl template build` so templates can opt into dev. Today template builds hardcode `BuildMode::Prod` in two `dev_build` call sites in `vm/template/lifecycle.rs`. The function-internal call signatures need a `mode` parameter threaded through; non-trivial because templates are stored artifacts and the build-time mode needs to round-trip with the saved spec.
- [ ] **W7.x.2 — microsandbox-as-Linux-builder Wave 2 (real impl)**. Fill in `StubBuilderVm` → `MicrosandboxBuilderVm`: pull the pinned OCI image (cached in `~/.cache/mvm/builder-image/`), spawn a microsandbox sandbox with `BuilderMounts` (flake src read-only at `/work`, host nix store at `/nix`, writable artifact dir at `/out`), run `nix build` inside, extract artifacts back to host. Wire detection in `dev_build.rs` (or a higher-level dispatcher) — when host Nix is unavailable, fall through to the builder VM. This is the change that makes the docs Nix sweep's "no host Nix needed" promise real.
- [ ] **CloudHypervisor real lifecycle** — `mvm-backend/src/cloud_hypervisor.rs` is currently a stub. JSON-API + spawn dance + start/stop/list/status implementations. Once real, wire `record_from_rootfs` into its `start` impl (same pattern as the other 4 backends).
