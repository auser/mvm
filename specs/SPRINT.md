...
- [x] **W8.A + W8.B — Firecracker stack relocation** ✅ — landed on `feat/w8-firecracker-direct-launch` (4 commits, 2026-05-10). The split scope reflects what was actually load-bearing vs. dead:
    - **W8.A** (`ccfb27d`) deleted truly-unreachable Lima symbols that W7 missed: `linux_env::LimaEnv` + impl, `config::{render_lima_yaml, render_lima_yaml_with, LimaRenderOptions, find_lima_template}`, `config::LEGACY_VM_NAME`, `resources/lima.yaml.tera`, `mvm-runtime`'s `tera` dep, plus updates to stale "inside the Lima VM" doc comments. Revealed that the runtime path was *already* Lima-free since W7 — `create_linux_env` returns `NativeEnv` or `AppleContainerEnv`, never Lima — so the "Firecracker direct-launch rewrite" framed in the original W8 description was unnecessary. The shell + linux_env + Firecracker stack already runs on the host directly via `bash -c`.
    - **W8.B** (`46e19cb` + `4cec0c5`) finished the architectural goal: every concrete `VmBackend` impl now lives in `mvm-backend`. Substrate moved to `mvm-runtime-base` (`config`, `shell/`, `linux_env`, plus a new `snapshot_integrity` lifted from `template::lifecycle`); the FC stack moved to `mvm-backend` (`firecracker`, `microvm`, `microvm_nix`, `network`, `image`, `backend`); 17 files in `mvm-cli` + 3 in `mvm-runtime` + 2 root-tree smoke tests migrated to `mvm_backend::*` imports; `mvmctl::backend` facade re-export added so the public surface mirrors `mvmctl::core` / `mvmctl::runtime`.
    - Re-exports kept for back-compat: `mvm_runtime::{config, linux_env, shell, ui, shell_mock}` (mvmd consumes `mvmctl::runtime::shell` etc. from ~30 files) and `mvm_runtime::vm::{cow, runtime_meta}` (mvmd's W6.2 console gate). Removing them would force a sibling-repo update.
...
- [ ] **W8.C — Wire non-Apple-Container `mvmctl dev`**. W7 slice 2 deleted the Lima-only `dev_up`/`dev_down`/`dev_status` helpers and replaced the dispatcher's else branches with `bail_no_dev_backend()` for non-AC hosts. With the W8.B FC stack relocation done, the path is open to wire `mvmctl dev` on Linux+KVM against a direct Firecracker dev image (existing `mvm_backend::firecracker::install`/`download_assets`/`prepare_rootfs` already runs natively on Linux now). On macOS Intel / pre-26 / no-KVM Linux, the `mvmctl dev` path probably stays "use the microsandbox builder VM" (W7.x.2's deliverable). Trivial-ish on Linux; design call on macOS Intel.
- [ ] **W6.1.2** — cross-compile the real `mvm-guest-agent` Rust binary (~2400 LOC) for target Linux and bake it into the rootfs. The W6.1.1 supervision pattern is in; only the binary swap is missing. Needs a Linux builder. **Unlocked once W7.x.2 lands** (in-VM builder removes the host-Nix dep).
