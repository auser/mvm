...
#### Land the in-flight work (commits)
- [ ] Stage and commit the 11 changesets. Suggested split: (1) plan 62 sidebar+placeholders, (2) Lima dead-branch cleanup, (3) W6.2 console gate skeleton (runtime_meta + console.rs + tests), (4) W7.x.1 builder_vm scaffolding, (5) W6.2/W7 sidecar bridge (ArtifactSidecar + from_sidecar), (6) W6.2 microsandbox consumer, (7) W6.2.1 dev_build producer, (8) W6.2.1 mvmd HostBackend producer, (9) W6.2.1 cross-backend consumer (record_from_rootfs + 3 backend call sites), (10) W6.2.2 BuildMode threading, (11) W6.2.3 --dev/--prod flags.
...
- [ ] **W7 — backend extraction**. Move `config`/`shell`/`ui`/`vm::microvm`/`vm::image` modules from `mvm-runtime` down to a shared crate (likely `mvm-runtime-base`); then physically relocate the backend impls (`firecracker.rs`, `microsandbox.rs`, `apple_container.rs`, `libkrun.rs`, `cloud_hypervisor.rs`, `docker.rs`) into the `mvm-backend` crate (today it's just a re-export façade). Also lands the internal handle registry that gives `StartMode::Attached` real SIGTERM-on-Ctrl-C semantics (today's `mode.json` is intent-only metadata).
...
- [ ] **CloudHypervisor real lifecycle** — `vm/cloud_hypervisor.rs` is currently a stub. JSON-API + spawn dance + start/stop/list/status implementations. Once real, wire `record_from_rootfs` into its `start` impl (same pattern as the other 4 backends).

#### Smaller cleanup items (mechanical, low-risk)
- [ ] Prune `bootstrap::is_lima_required()` (one-line wrapper, no callers after Lima cleanup).
- [ ] Remove the `vm::lima` no-op shim file once nothing imports `lima::*` symbols (audit `mvm-cli` for stale `use mvm_runtime::vm::lima` imports — earlier wave caught most; do a final sweep).
- [ ] `inside_lima()` in `shell/exec.rs` — heuristic that's now permanently false (no Lima env). Either delete or document as a no-op.
