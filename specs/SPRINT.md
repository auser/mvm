...
- **W7** — finish the backend extraction: move `config`/`shell`/`ui`/`vm::microvm`/`vm::image` modules from `mvm-runtime` down to a shared crate (likely `mvm-core` or a new `mvm-runtime-shared`); then physically relocate the backend impls into `mvm-backend` (today they live in `mvm-runtime/src/vm/` and the `mvm-backend` crate is a thin re-export). Tracked here so the layering is intentional, not accidental.
