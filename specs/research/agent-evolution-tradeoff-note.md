...
Plan 104 (host services broker over vsock, merged 2026-05-26), ADR-059 (broker architecture), ADR-061 (broker hardening), and **ADR-062 (merged 2026-05-28, drops secrets, adds `host.audit.v1`)** broaden the vsock surface from a single host↔agent control channel to **two named ports**, all sharing the same `AuthenticatedFrame` envelope. Any agent-evolution decision must account for the full set:
...
| **5300** | `mvm-broker` subprocess (uid 903, ADR-062 3-subprocess model) | mvm SDK in workload (or agent, for built-in services) | `AuthenticatedFrame` → `ServiceCall`/`ServiceResponse` | Broker: `host.time.v1`, `host.cost.v1`, `host.audit.v1`, `broker.v1`. `mvm-host-signer` (uid 904) and `mvm-audit-signer` (uid 905) sit behind per-VM UDS — no vsock listeners. |
| ~~**5301**~~ | ~~removed by ADR-062~~ | n/a | n/a | Was the secrets channel under pre-ADR-062 design. `mvm-secrets-dispatcher` + `host.secrets.v1` dropped. **Do not reference in new specs.** |
...
- **W3 (Noise_NK encryption design)** must cover **both guest↔host vsock connections** (5252 + 5300), not just the agent control channel. The same host static Ed25519 pubkey (already baked in via `mkGuest { hostPubkey = ...; }`) authenticates both. Each port runs its own session — two independent Noise_NK handshakes, same key material.
- **W4 ADR "control-plane vs data-plane"** must name the broker explicitly as part of the control plane. Workload→host service calls (time, cost, audit-emit) are control-plane traffic — they cross the boundary, carry audit-emitting RPCs, and gate on the signed `ExecutionPlan.services` bindings. They are **not** data plane (D3) — the data plane is workload bytes (stdout, network, fs); broker calls are workload→host *service* invocations.
...
- **W5 capability matrix** column "vsock-encryption support" must be evaluated against the *multi-port* surface — a backend that supports vsock at all supports both ports (Firecracker, libkrun, Apple VZ, Cloud Hypervisor), so this is uniform — but the matrix should note the breadth.
- **I1 (control-plane audit chain)** spans the broker too. Plan 104 funnels all broker calls (including the new `host.audit.v1` workload-emit path) through the supervisor's `AuditEmitter` via the `mvm-audit-signer` subprocess so the chain stays linear. Any agent change must preserve this contract — but the broker work has already done the audit integration; W1 inherits it.
- **Process-supervision discipline still relevant.** The pre-ADR-062 `mvm-secrets-dispatcher` is gone, but the 3-subprocess broker model (`mvm-broker` + `mvm-host-signer` + `mvm-audit-signer`, each with isolated address space) is the same architecture pattern the agent itself could adopt (split monolith → core dispatcher + composable handlers, possibly out-of-process for the most security-critical verbs). The lean Rust v2 agent-v2 follow-on should learn from the broker's multi-process design.
...
- The encryption design (W3) is unchanged in *shape* — Noise_NK still fits; it now applies to two ports instead of one. Lean Rust v2 with `snow` handles two ports as easily as one; Zig from-scratch implementation gets two handshake state machines (small additional cost).
...
- [Plan 104 — Host services broker over vsock](../plans/104-host-services-broker.md) (merged 2026-05-26, rescoped 2026-05-28 by ADR-062 — adds vsock port :5300 hosting `host.time.v1`, `host.cost.v1`, `host.audit.v1`, `broker.v1` via the 3-subprocess model (`mvm-broker`, `mvm-host-signer`, `mvm-audit-signer`); same `AuthenticatedFrame` envelope; W3 encryption design must cover the broker surface, not just the agent's :5252)
- [ADR-062 — Drop host.secrets.v1, add host.audit.v1](../adrs/062-host-services-broker-rescope-drop-secrets.md) (merged 2026-05-28 — supersedes ADR-049, narrows ADR-059 and ADR-061; this note's broker-surface table reflects ADR-062's post-rescope state)
