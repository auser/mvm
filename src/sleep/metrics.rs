...
pub struct IdleMetrics {
    /// Seconds since last meaningful guest activity.
    pub idle_secs: u64,
    /// CPU usage percentage (0-100) over the last sampling window.
    pub cpu_pct: f32,
    /// Network bytes in the last sampling window.
    pub net_bytes: u64,
...
pub fn collect_metrics(instance_dir: &str, socket_path: &str) -> Result<IdleMetrics> {
    ...
    Ok(IdleMetrics {
        ...
        cpu_pct,
        net_bytes,
        last_updated: Some(http::utc_now()),
    })
}
