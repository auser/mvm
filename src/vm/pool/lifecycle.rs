...
use super::config::{DesiredCounts, InstanceResources, PoolSpec, Role, pool_config_path, pool_dir};
        ...
        && let Ok(instances) = crate::vm::instance::lifecycle::instance_list(tenant_id, pool_id)
    {
        let active_count = instances
            ...
            .filter(|i| {
                matches!(
                    i.status,
                    crate::vm::instance::state::InstanceStatus::Running
                        | crate::vm::instance::state::InstanceStatus::Warm
                )
            })
            .count();
        if active_count > 0 {
